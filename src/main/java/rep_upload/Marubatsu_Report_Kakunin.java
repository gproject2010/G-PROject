...
/**
 * Servlet implementation class Marubatsu_Report_Kakunin
 */
public class Marubatsu_Report_Kakunin extends HttpServlet {
	private static final long serialVersionUID = 1L;
    
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Marubatsu_Report_Kakunin() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		session.setAttribute("errcode", "Rep-005");
		String NextPage = "/passed/Error_Gamen.jsp";
		boolean over = false;
		boolean notscore = false;
		boolean wrong = false;
		boolean few = false;//マイナスの値が入力されているかを表すフラグ
		String NGWord = null;//NGワードが含まれていることを示すフラグ
...
String Dificultty_S = request.getParameter("difficulty"); //難易度の適正度
		...
		if(Jisu_Wrong || over || few || NGWord != null || notmatch || notOutline || notGoodies || notscore || wrong || notAnswer){
			
			NextPage = "/passed/OX_Report/MaruBatsu_Syusei.jsp";
		}else{	
			NextPage = "/passed/OX_Report/MarubatsuRep_Kakunin.jsp";
		}
	}catch(Exception ex){
		ex.printStackTrace();
		session.setAttribute("errcode", "Rep-005");
