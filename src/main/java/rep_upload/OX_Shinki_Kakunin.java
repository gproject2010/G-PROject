...
/**
 * Servlet implementation class OX_Shinki_Kakunin
 */
public class OX_Shinki_Kakunin extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public OX_Shinki_Kakunin() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		session.setAttribute("errcode", "Rep-007");
		...
		try{
			...
			boolean notGoodies = false;
			boolean notBadies = false;
		...
		String Hatsubainen = request.getParameter("hatsubainen");//発売年
		String Hatsubaitsuki = request.getParameter("hatsubaitsuki");//発売月
		String Hatsubaibi = request.getParameter("hatsubaibi");//発売日
		String Maingenre = request.getParameter("Maingenre");//メインジャンル
		String Platform = request.getParameter("Platform");//プラットフォーム
		
		String Hatsubainengappi = "0001-01-01";
		
		GS_jyouhou_TO Gamedata = null;
				
		boolean Notitle = false;
		String NGWord = null;;//NGワードが含まれていることを表すフラグ
		
			
		if(Gametitle == null || Gametitle.equals("") || Gametitle.equals("もう一度入力してください")){
			Gametitle = "もう一度入力してください";
			Notitle = true;
		}else if(Gametitle.length() > 40){
			Jisu_Wrong = true;
		}
		//Gametitle = HttpUtility.escapeHTML(Gametitle);
		if(HttpUtility.NG_Word_Check(Gametitle) != null){
			NGWord = HttpUtility.NG_Word_Check(Gametitle);//NGワードチェック
		}
				
		if(Makername == null || Makername.equals("")){
			Makername = "不明";
		}else if(Makername.length() > 20){
			Jisu_Wrong = true;
			//Makername = HttpUtility.escapeHTML(Makername);
		}
			if(HttpUtility.NG_Word_Check(Makername) != null){
				NGWord = HttpUtility.NG_Word_Check(Makername);//NGワードチェック
			}
				
		if(Hatsubainen == null || Hatsubainen.equals("") || Hatsubainen.equals("0001")){
			Hatsubainengappi = "0001-01-01";
		}else if(Hatsubainen.length() > 4 || Hatsubaitsuki.length() > 2 || Hatsubaibi.length() > 2){
			Jisu_Wrong = true;
			//Hatsubainengappi = HttpUtility.escapeHTML(Hatsubainengappi);
		}else{
			Hatsubainengappi = Hatsubainen + "-" + Hatsubaitsuki + "-" + Hatsubaibi;
		}
			if(HttpUtility.NG_Word_Check(Hatsubainengappi) != null){
				NGWord = HttpUtility.NG_Word_Check(Hatsubainengappi);//NGワードチェック
			}
				
		if(Maingenre == null || Maingenre.equals("")){
			Maingenre = "不明";
		}else if(Maingenre.length() > 30){
			Jisu_Wrong = true;
			//Maingenre = HttpUtility.escapeHTML(Maingenre);
		}
		if(HttpUtility.NG_Word_Check(Maingenre) != null){
				NGWord = HttpUtility.NG_Word_Check(Maingenre);//NGワードチェック
		}
		...
		if(HttpUtility.NG_Word_Check(Platform) != null){
				NGWord = HttpUtility.NG_Word_Check(Platform);//NGワードチェック
			}
		
		
		Gamedata = new GS_jyouhou_TO(Notitle,  Gametitle, Makername, Hatsubainengappi, Maingenre, Platform);
		session.setAttribute("indata", Gamedata);
...
String Dificultty_S = request.getParameter("difficulty"); //難易度の適正度
		...
		String goodies = request.getParameter("goodies");//良かった所
		
		if(goodies == null || goodies.equals("") || goodies.equals("もう一度入力してください")){
			goodies = "もう一度入力してください";
			notGoodies = true;
		}else if(goodies.length() > 500){
			Jisu_Wrong = true;
		}
			//goodies = HttpUtility.escapeHTML(goodies);
			//goodies = goodies.replaceAll("\n", "<br>");
			if(HttpUtility.NG_Word_Check(goodies) != null){
				NGWord = HttpUtility.NG_Word_Check(goodies);//NGワードチェック
			}
		
		String badies = request.getParameter("badies");//悪かった所
		if(badies == null || badies.equals("")){
			badies = "記述なし";
		}else if(badies.length() > 500){
			Jisu_Wrong = true;
		//badies = HttpUtility.escapeHTML(badies);
		//badies = badies.replaceAll("\n", "<br>");
		if(HttpUtility.NG_Word_Check(badies) != null){
			NGWord = HttpUtility.NG_Word_Check(badies);//NGワードチェック
		}
		}
...
String Konyu_Houhou = request.getParameter("konyu_houhou");//購入方法
		...
		Judge = new Rep_Koumoku(Jisu_Wrong, over, notscore, wrong, few, NGWord, notmatch, dificultty, story, graphic, sound, volume, controll, promotion, free_S, total, Jiyuten_Koumoku1, Jiyuten_Koumoku2, Jiyuten_Koumoku3, Jiyuten_Koumoku4, Jiyuten_Koumoku5, Jiyuten_Score1, Jiyuten_Score2, Jiyuten_Score3, Jiyuten_Score4, Jiyuten_Score5,
				Jiyuten_ScoreTotal, Jiyuten_MaxScore1, Jiyuten_MaxScore2, Jiyuten_MaxScore3, Jiyuten_MaxScore4, Jiyuten_MaxScore5, Jiyuten_MaxScoreTotal, outline, notOutline, goodies, notGoodies, badies, notBadies, Konyu_Houhou,
				Interview1, Interview2, Interview3, Interview4, Interview5, Interview6, notAnswer);
		session.setAttribute("hyouka", Judge);
		
		if(Jisu_Wrong || Notitle || few || NGWord != null || notmatch  || over || notOutline || notGoodies || notscore || wrong || notAnswer){
			
			NextPage = "/passed/OX_Report/OX_Shinki_Syusei.jsp";
		}else{	
			NextPage = "/passed/OX_Report/OX_Shinki_Kakunin.jsp";
		}
	}catch(Exception ex){
		ex.printStackTrace();
		session.setAttribute("errcode", "Rep-007");
