...
<br>
<form action="/G-pro_Service/FreeRep_Download" method="post">
<input type="hidden" name="downloadfilename" value="<%=bestrep.getUploadFileName() %>">
<input type="image" name="submit" src="/G-pro_Service/image/Rep_DL_Button.gif" width="175" height="46" border="0" alt="レポートデータ 呼び出し">
</form>
<br>
<br>
...
<table border="1">
  <tbody>
    <tr>
      ...
      <td><% if(vote.getYourVote() != null && vote.getYourVote().equals("empty")){ %><a href="/G-pro_Service/Rep_Vote_Upload?rep_code=<%=bestrep.getreport_code() %>&command=fushiji"><% } %><img src="/G-pro_Service/image/Bad_Icon.gif" width="106" height="52" border="0"><font size="+1"><b>×<%=vote.getFushijisyasu() %></b></font><% if(vote.getYourVote() != null && vote.getYourVote().equals("empty")){ %></a><% } %></td>
    </tr>
  </tbody>
</table>
<% if(!(vote.getYourVote().equals("empty"))){ %><p><font color="#ff0000">※あなたはこのレポートに投票済みです。</font></p><% } %>
