...
<html lang="ja">
<head>
<base href="/G-pro_Service/passed/Kiji_Report/Kiji_Rep_Syusei.jsp"/>
...
<title>紹介記事レポート(ゲーム情報からの登録用)</title>
</head>
...
<br>
サブジャンル2：<%=HttpUtility.escapeHTML(syousai.getSubgenre2())%>　　　　　プラットフォーム：<%=HttpUtility.escapeHTML(syousai.getPlatform())%>
<br>

評価点数(必須)※整数または第2位以内の小数で入力してください</p>
<form action="/G-pro_Service/Kiji_Report_Kakunin" method="post">
<table border="1" align="left">
  <tbody>
    ...
    <tr>
      <td>&nbsp;難易度の適正度</td>
      <td>
      <input size="5" type="text" maxlength="5" name="difficulty" id="dificultty" class="score" value="<%=kijihyouka.getdificultty() %>">点
      </td>
      <td>&nbsp;/10点</td>
    </tr>
    <tr>
      ...
      <td>
      <input size="5" type="text" maxlength="5" name="story" id="story" class="score" value="<%=kijihyouka.getstory() %>">点
      </td>
      <td>&nbsp;/10点</td>
    </tr>
    <tr>
      <td>&nbsp;グラフィック </td>
      <td>
      <input size="5" type="text" maxlength="5" name="graphic" id="graphic" class="score" value="<%=kijihyouka.getgraphic() %>">点
      </td>
    ...
    <tr>
      <td>&nbsp;サウンド</td>
      <td>
      <input size="5" type="text" maxlength="5" name="sound" id="sound" class="score" value="<%=kijihyouka.getsound() %>">点
      </td>
      <td>&nbsp;/10点</td>
    </tr>
    <tr>
      <td>&nbsp;ボリューム度・ハマり度</td>
      <td>
      <input size="5" type="text" maxlength="5" name="volume" id="volume" class="score" value="<%=kijihyouka.getvolume() %>">点
      </td>
      <td align="center">/10点&nbsp;</td>
    </tr>
    <tr>
      <td>&nbsp;操作性</td>
      <td>
      <input size="5" type="text" maxlength="5" name="controll" id="promotion" class="score" value="<%=kijihyouka.getcontroll() %>">点
      </td>
      <td align="center">&nbsp;/10点</td>
    </tr>
    <tr>
      <td>&nbsp;プロモーション</td>
      <td>
      <input size="5" type="text" maxlength="5" name="promotion" id="jiyuten_set" class="score" value="<%=kijihyouka.getpromotion() %>">点
      </td>
      <td>/10点&nbsp;</td>
    </tr>
    <tr>
      <td>&nbsp;提出者の考えによる自由点</td>
      <td>
      <span id="totalscore_hyouji"><%=kijihyouka.gettotal() %></span>点
...
<br>
ゲームの概要・ストーリー(必須)<br>
<textarea rows="5" cols="100" name="outline" id="count_area1" maxlength="500"><%=HttpUtility.escapeHTML(kijihyouka.getoutline())%></textarea><br>
...
<%}else{%>
あなたの回答：<%for(String Inter1 : kijihyouka.getInterview1()){ %><%=HttpUtility.escapeHTML(Inter1) + ", " %><br>
<% } %>
<% } %>
<br>
...
<% } %>
<br>
<input size="45" type="text" maxlength="50" name="midashi" value="<%=HttpUtility.escapeHTML(kijihyouka.getmidashi())%>"><br>
※全角40文字以内で入力してください。<br>
<p align="left"><font size="+2">自分が思うこのゲームの特徴・セールスポイント</font>(必須)</p>
<textarea rows="5" cols="100" name="feature" id="count_area2" maxlength="500"><%=HttpUtility.escapeHTML(kijihyouka.getfeature())%></textarea>　<br>
※全角500文字以内(あと<span id="mojisu2">---</span>文字)<br>
<br>
<font size="+2">このゲームの評価</font>(必須)
<textarea rows="5" cols="100" name="evaluate" id="count_area3" maxlength="500"><%= HttpUtility.escapeHTML(kijihyouka.getevaluate())%></textarea><br>
※全角500文字以内(あと<span id="mojisu3">---</span>文字)<br>
<input type="hidden" name="shinkiflg" value="false">
<!-- <p align="center"><input type="image" src="/G-pro_Service/Kakutei_Button.gif" width="61" height="48" border="0" alt="確定" name="submit">　　　<a href="/G-pro_Service/passed/G_SeachResult.jsp"><img src="/G-pro_Service/passed/GData_Images/BacktoSerchResult.gif" alt="検索結果に戻る"></a>　　　<a href="/G-pro_Service/passed/Kaiin_Menu_Top.jsp"><img src="/G-pro_Service/passed/GData_Images/BacktoKaiinTop.gif" width="149" height="43" border="0"></a></p>-->
